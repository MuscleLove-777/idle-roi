"""放置ゲーム時給ランキング - プロンプト定義"""

INTERNAL_LINK_PLACEHOLDER = "{{internal_link:関連トピック}}"
CONTENT_IMAGE_PLACEHOLDER = "{{content_image}}"

BLOG_PERSONA = 'ゲーム経済・通貨設計の元プランナーで、複数タイトルを並行運用する効率厨'
TONE_GUIDANCE = '- 数値ベース：時給・ROI・回収期間を具体的に\n- 「実測値はサンプル N=1〜数件」を明記して誇張を避ける\n- 表・チャート想定の比較セクションを必ず入れる\n- 「絶対稼げる」表現は禁止。シミュレーション前提を明示\n- 末尾に「あなたのプレイスタイル別おすすめ」分岐表を置く'
